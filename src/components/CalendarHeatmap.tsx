...
function dotColor(count: number): string {
  if (count === 0) return ''
  if (count <= 3) return 'bg-accent-500/40'
  if (count <= 7) return 'bg-accent-500/60'
  if (count <= 15) return 'bg-accent-500/80'
          ...
          return (
            <button
              ...
              className={`
                ...
                transition-colors duration-150 cursor-pointer select-none rounded
                ${isSelected
            ...
            >
              {cell.day}
              {cell.count > 0 && (
                <span className={`absolute bottom-0.5 w-1 h-1 rounded-full ${dot}`} />
              )}
            </button>
          )
        })}
